...
extension PerformanceTestsViewController {
    ...
    @IBAction func performNotificationCenterTests_OneReceiver() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            let sendCount: Int64 = 1_000_000
            var receiveCount: Int64 = 0
            let notificationID = "TestNotificationID1"

            let startTime = CFAbsoluteTimeGetCurrent()

            let observer = NotificationCenter.default.addObserver(forName: Notification.Name(rawValue: notificationID), object: nil, queue: nil) { _ in
                receiveCount += 1
